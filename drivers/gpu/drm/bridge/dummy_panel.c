...
#define DUMMY_PANEL_I2C_ADDRESS	0x31

//static inline struct dummy_panel_device
//*sd_to_dummy_panel(struct dummy_panel_device *sd)
//{
	//return container_of(sd, struct dummy_panel_device, sd);
//}

//static inline struct dummy_panel_device
//*i2c_to_dummy_panel(struct i2c_client *client)
//{
	//return sd_to_dummy_panel(i2c_get_clientdata(client));
//}

static int dummy_panel_initialize(struct dummy_panel_device *dev)
{
	/* Implement as needed */
	return 0;
}
...
static int dummy_panel_probe(struct i2c_client *client)
{
	struct dummy_panel_device *dummy_dev;
	int ret;
	
	dummy_dev = devm_kzalloc(&client->dev, sizeof(*dummy_dev), GFP_KERNEL);
	if (!dummy_dev)
		return -ENOMEM;
		
	dummy_dev->dev = &client->dev;
	
	/*
	* Create the dummy I2C client for the panel: the right i2c address
	* will be overwritten later at panel initialization time.
	*/
	dummy_dev->panel_dummy = i2c_new_dummy_device(client->adapter, DUMMY_PANEL_I2C_ADDRESS);
	if (!dummy_dev->panel_dummy) {
		ret = -ENXIO;
		goto error;
	}

	/* Initialize the hardware. */
	ret = dummy_panel_initialize(dummy_dev);
	if (ret < 0)
		goto error;
		
	/* Register the panel. */
	drm_panel_init(&dummy_dev->panel, dummy_dev->dev, &dummy_panel_funcs,
					DRM_MODE_CONNECTOR_VIRTUAL);
		       
	drm_panel_add(&dummy_dev->panel);
	
	i2c_set_clientdata(client, dummy_dev);
	
error:
	if (dummy_dev->panel_dummy)
		i2c_unregister_device(dummy_dev->panel_dummy);
	dev_err(&client->dev, "probe failed\n");
	return ret;
}

static int dummy_panel_remove(struct i2c_client *client)
{
	//struct dummy_panel_device *dev = i2c_to_dummy_panel(client);

	//i2c_unregister_device(dev->panel_dummy);

	return 0;
}

static const struct of_device_id dummy_panel_of_ids[] = {
	{ .compatible = "dummy,panel", },
	{ }
};
MODULE_DEVICE_TABLE(of, dummy_camera_of_ids);

static struct i2c_driver dummy_panel_i2c_driver = {
	.driver	= {
		.name	= "dummy_panel",
		.of_match_table = dummy_panel_of_ids,
	},
	.probe_new	= dummy_panel_probe,
	.remove		= dummy_panel_remove,
};

module_i2c_driver(dummy_panel_i2c_driver);

MODULE_ALIAS("dummy_panel");
MODULE_DESCRIPTION("GMSL2 dummy panel driver");
MODULE_LICENSE("GPL");
